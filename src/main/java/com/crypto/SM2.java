...
public class SM2 {
	 ...
	 * @return
	 */
	public ECPoint getPublicKeyFromBytes(byte[] bytes){
	 ...
	 * @return 签名
	 */
	public Signature signByte(byte[] M_, String IDA, SM2KeyPair keyPair) {
		byte[] ZA = ZA(IDA, keyPair.getPublicKey());
		BigInteger e = new BigInteger(1, sm3hash(M_));
		BigInteger k;
		BigInteger r;
	 ...
	 * @return
	 */
	public boolean verifyBytes(byte[] M_,byte[] r,byte[] s, String IDA, ECPoint aPublicKey) {
		BigInteger SR=new BigInteger(r);
		BigInteger SS=new BigInteger(s);
		if (!between(SR, BigInteger.ONE, n))
			return false;
		if (!between(SS, BigInteger.ONE, n))
			return false;

		BigInteger e = new BigInteger(1, sm3hash(M_));
		BigInteger t = SR.add(SS).mod(n);

		if (t.equals(BigInteger.ZERO))
			return false;

		ECPoint p1 = G.multiply(SS).normalize();
		ECPoint p2 = aPublicKey.multiply(t).normalize();
		BigInteger x1 = p1.add(p2).normalize().getXCoord().toBigInteger();
		BigInteger R = e.add(x1).mod(n);
		if (R.equals(SR))
	 ...
	 * @return
	 */
	public boolean verify2(String M,byte[] r,byte[] s, String IDA, ECPoint aPublicKey) {
		BigInteger SR=new BigInteger(r);
		BigInteger SS=new BigInteger(s);
		if (!between(SR, BigInteger.ONE, n))
			return false;
...
//		sm02.exportPublicKey(publicKey, "publickey.pem");
//		sm02.exportPrivateKey(privateKey, "privatekey.pem");


//		System.out.println("-----------------公钥加密与解密-----------------");
//		ECPoint publicKey = sm02.importPublicKey("E:/test/publickey.pem");
//		BigInteger privateKey = sm02.importPrivateKey("E:/test/privatekey.pem");
//		byte[] data = sm02.encrypt("吃饭睡觉打豆豆！！", publicKey);
//		System.out.print("密文:");
...
//		System.out.println("验证签名:" + sm02.verify(M, signature, IDA, publicKey));
		...
		System.out.println(aKeyPair.getPublicKey());
		System.out.println("+++++++++++++++++++++++++++++++++++++++++++");
		System.out.println(bKeyPair.getPublicKey());
		System.out.println("+++++++++++++++++++++++++++++++++++++++++++");
